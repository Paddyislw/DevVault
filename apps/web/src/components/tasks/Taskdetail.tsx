...
import { Code2, Link2, FileText, Image } from "lucide-react";
...
export function TaskDetail({ description, attachments, subtasks }: TaskDetailProps) {
  const hasContent = description || attachments.length > 0 || subtasks.length > 0;
      ...
      {subtasks.length > 0 && (
        <div className="mt-3">
          ...
          <div className="space-y-1">
            {subtasks.map((sub) => (
              <div key={sub.id} className="flex items-center gap-2 text-[13px] text-text-secondary">
                <div
                ...
                />
                <span className={sub.status === "DONE" ? "line-through text-text-ghost" : ""}>
                  {sub.title}
                </span>
              </div>
            ))}
